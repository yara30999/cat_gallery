...
class VotesPaginationGridBuilder extends StatefulWidget {
  final List<CatWithClickEntity> catImages;
  const VotesPaginationGridBuilder({super.key, required this.catImages});

  @override
  State<VotesPaginationGridBuilder> createState() =>
      _VotesPaginationGridBuilderState();
}

class _VotesPaginationGridBuilderState
    extends State<VotesPaginationGridBuilder> {
  var isLoading = false;
  var nextPage = 1;
  var previousIndex = 0;
  ...
  void _fetchMoreData(String uid) {
    isLoading = true;
    BlocProvider.of<VotesCubit>(context)
        .getVotes(uid: uid, pageNum: nextPage++);
    isLoading = false;
  }
